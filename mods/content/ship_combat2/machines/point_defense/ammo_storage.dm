...
/obj/machinery/point_defense/ammo_storage/on_update_icon()
	...
	if(!ammo)
		overlays += image('mods/content/ship_combat2/icons/pointdefensemainframe.dmi', "pdc_ammo_[orientation]_empty")

	if(ammo <= max_ammo * 0.25)
		overlays += image('mods/content/ship_combat2/icons/pointdefensemainframe.dmi', "pdc_ammo_[orientation]3")

	if(ammo <= max_ammo * 0.50)
		overlays += image('mods/content/ship_combat2/icons/pointdefensemainframe.dmi', "pdc_ammo_[orientation]2")

	if(ammo <= max_ammo * 0.75)
		overlays += image('mods/content/ship_combat2/icons/pointdefensemainframe.dmi', "pdc_ammo_[orientation]1")

	if(ammo == 100)
		overlays += image('mods/content/ship_combat2/icons/pointdefensemainframe.dmi', "pdc_ammo_[orientation]")
		return
