...
/datum/interaction/headpat
	...
	sounds = list('starlight/mods/content/interaction/sound/hug.ogg')
...
/datum/interaction/slap
	...
	id = "slap"
	user_flags = INT_HAN | INT_CONS | INT_CUFF
	sounds = list('starlight/mods/content/interaction/sound/slap.ogg')
	color = "red"
...
/obj/effect/decal/cleanable/cum
	...
	icon = 'starlight/mods/content/interaction/icons/cum.dmi'
...
/datum/interaction/sex/proc/cum(var/mob/living/carbon/human/H, var/mob/living/carbon/human/P,var/flag)
	...
	if(H.species.name != SPECIES_FRAME) playsound(get_turf(H), "starlight/mods/content/interaction/sound/final_[H.gender == MALE ? "m" : "f"][H.gender == MALE ? rand(1,5) : rand(1,3)].ogg", 40 + (H.gender == FEMALE ? 40 : 0), 1, frequency = H.get_age_pitch())
...
/datum/interaction/sex/proc/moan(var/mob/living/carbon/human/H)
	...
	if((H.species.name in MOAN_SPECIES_LIST) && prob(H.lust / H.max_lust * (H.gender == MALE ? 5 : 60)))
		...
		if(!istype(H.loc, /obj/structure/closet)) 
			playsound(get_turf(H), "starlight/mods/content/interaction/sound/moan_[gend_sound][moan_sound].ogg", 70, 1, frequency = H.get_age_pitch())
